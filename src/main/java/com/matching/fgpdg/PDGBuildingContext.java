...
public class PDGBuildingContext {
    ...
    private TypeWrapper typeWrapper; //TODO: remove? -> this contains the guards as well!!!
...
//    protected boolean interprocedural;
    ...
    public PDGBuildingContext(List<ImportDeclaration> importStmt, String sourceFilePath) throws IOException {
    ...
    public PDGBuildingContext(List<ImportDeclaration> importStmt) {
        updateImportMap(importStmt);
    }

    public PDGBuildingContext(List<ImportDeclaration> importStmt, TypeWrapper typeWrapper) {
    ...
    //TODO: how to handle 'package.*'-s???
    private void updateImportMap(List<ImportDeclaration> importStmt) {
        for (Object anImport : importStmt) {
            if (anImport instanceof ImportDeclaration im) { //TODO: maybe fullyQualifiedName is needed also???
                if (im.getasName() != null) {
                    importsMap.put(im.getasName().toString(), im.getName().toString());
                } else {
                    String[] split = im.getName().toString().split("\\.");
                    importsMap.put(split[split.length - 1], im.getName().toString());
                }
            }
...
//            }
        ...
        if (this.typeWrapper.getGuards() != null) {
            importsMap.putAll(this.typeWrapper.getGuards().getImports());
    ...
    public String getKey(ArrayAccess astNode) {
        ...
        if (a instanceof ArrayAccess) {
            ...
            if (info != null)
                name = info[0];
//            else
//                name = getTypeWrapper().getTypeInfo(((Name) a), ((Name) value).getCol_offset(), ((Name) value).getInternalId());
